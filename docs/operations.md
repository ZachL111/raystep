...
`raystep` is centered on graphics work in C++. The technical target is: Render signed-distance scenes to PPM fixtures with camera controls.
...
The score is deterministic. The current threshold is 165. Risk has a penalty of 4, latency has a penalty of 2, and weight contributes a bonus of 3.
